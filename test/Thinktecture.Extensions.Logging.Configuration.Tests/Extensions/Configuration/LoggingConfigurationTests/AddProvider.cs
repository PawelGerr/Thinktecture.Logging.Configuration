...
namespace Thinktecture.Extensions.Configuration.LoggingConfigurationTests
{
	public class AddProvider : TestBase
	{
		...
		{
			ProviderMock.Setup(p => p.ResetLevel(null, null));

			var config = CreateConfig(out var collection);

			config.ResetLevel();
			ProviderMock.Verify(p => p.ResetLevel(null, null), Times.Never);

			collection.Add(ProviderMock.Object);

			config.ResetLevel();
			ProviderMock.Verify(p => p.ResetLevel(null, null), Times.Once);
		}
	}
}
